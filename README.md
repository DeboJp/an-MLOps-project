...
* **Retrieval is not the best but not the worst:** In Retrieved mode (where we ran the actual ensemble search), accuracy dropped off. The retriever likely pulled in distractors and numbers that the model got confused by or never pulled correct chunks. Our **retriever eval**-`check_recall.py` told us it was accurate only ~80% of the time and only 75% of the time grabbed all the correct chunks(gave full context) for the question.
