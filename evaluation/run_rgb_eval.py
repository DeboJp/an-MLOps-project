...
def evaluate_testbed(testbed_name, dataset_path, client, limit=50, show_comparison=False):
    ...
    if not os.path.exists(dataset_path):
        print(f"Error: Dataset {dataset_path} not found. Run generate_rgb.py first.")
        return None
...
def main():
    ...
    testbeds_dir = os.path.join(PROJECT_ROOT, "data", "rgb_eval")
    testbeds = {
        "noise_robustness": os.path.join(testbeds_dir, "noise_robustness.json"),
        "negative_rejection": os.path.join(testbeds_dir, "negative_rejection.json"),
        "information_integration": os.path.join(testbeds_dir, "information_integration.json"),
        "counterfactual_robustness": os.path.join(testbeds_dir, "counterfactual_robustness.json")
    }
